...
1. Clone `backend` to `/srv/villaone/backend` and `website` to `/srv/villaone/website`.
...
5. Start only PostgreSQL: `docker compose --env-file .env.production -f docker-compose.production.yml up -d postgres`.
