...
export default function SupportPage() {
  ...
  return <main dir="rtl" className={`${styles.page} inner-page support-page`}><InnerHeader /><header className="support-hero section-shell"><p className="eyebrow dark"><span /> همراه شما در سفر</p><h1>مرکز پشتیبانی ویلاوان</h1><p>برای رزرو، پرداخت، لغو یا هماهنگی ورود پیام بگذارید. درخواست شما مستقیم به تیم عملیات می‌رسد.</p></header><section className="support-layout section-shell">
    <article className="support-form"><h2>درخواست جدید</h2><div className="support-fields"><PublicSelect className="support-select" label="رزرو مرتبط" value={form.bookingCode} onChange={(bookingCode) => setForm({ ...form, bookingCode })} options={[{ value: "", label: "بدون رزرو مشخص" }, ...bookings.map((booking) => ({ value: booking.code, label: `${booking.title} · ${booking.code}` }))]} /><PublicSelect className="support-select" label="موضوع درخواست" value={form.category} onChange={(category) => setForm({ ...form, category: category as SupportTicket["category"] })} options={[{ value: "booking", label: "رزرو" }, { value: "payment", label: "پرداخت" }, { value: "cancellation", label: "لغو و بازگشت وجه" }, { value: "stay", label: "اقامت و ورود" }, { value: "other", label: "سایر" }]} /><label className="wide"><span>عنوان</span><input value={form.subject} onChange={(event) => setForm({ ...form, subject: event.target.value })} placeholder="خلاصه درخواست شما" /></label><label className="wide"><span>توضیحات</span><textarea rows={5} value={form.message} onChange={(event) => setForm({ ...form, message: event.target.value })} placeholder="جزئیاتی که به بررسی سریع‌تر کمک می‌کند..." /></label></div><button type="button" disabled={sending} onClick={submit}>{sending ? "در حال ارسال…" : "ثبت درخواست پشتیبانی"}</button>{notice && <p className="support-notice" role="status">{notice}</p>}</article>
