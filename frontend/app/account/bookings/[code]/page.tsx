...
export default function BookingReceiptPage() {
  ...
  return <main dir="rtl" className="inner-page receipt-page"><InnerHeader /><section className="receipt-shell section-shell">
    {error ? <div className="receipt-state"><h1>رسید در دسترس نیست</h1><p>{error}</p><a href="/account">بازگشت به حساب</a></div> : !booking ? <div className="receipt-state"><h1>در حال آماده‌سازی رسید…</h1></div> : <article className="receipt-paper">
      ...
      <div className="receipt-title"><p>کد رزرو</p><h1 dir="ltr">{booking.code}</h1><span className={`receipt-status ${booking.status}`}>{booking.status === "confirmed" ? "تأییدشده" : booking.status === "completed" ? "اقامت تکمیل‌شده" : booking.status === "cancelled" ? "لغوشده" : booking.status === "expired" ? "مهلت پرداخت تمام‌شده" : "در انتظار تأیید"}</span></div>
      <section className="receipt-villa"><img src={booking.villa.cover_image || "/images/villaone-hero.png"} alt={booking.villa.title} /><div><small>{booking.villa.city.name} · مازندران</small><h2>{booking.villa.title}</h2><p>{date(booking.checkin)} تا {date(booking.checkout)} · {booking.guests_count.toLocaleString("fa-IR")} مهمان</p></div></section>
      <section className="receipt-finance"><h2>خلاصه مالی</h2><dl><div><dt>هزینه اقامت</dt><dd>{money(Number(booking.stay_total) > 0 ? booking.stay_total : booking.total_price)} تومان</dd></div>{Number(booking.services_total) > 0 && <div><dt>خدمات انتخابی</dt><dd>{money(booking.services_total)} تومان</dd></div>}<div><dt>مبلغ کل رزرو</dt><dd>{money(booking.total_price)} تومان</dd></div><div><dt>پرداخت آنلاین</dt><dd>{money(booking.deposit_paid_online)} تومان</dd></div><div><dt>مانده قابل پرداخت</dt><dd>{money(booking.remaining_amount)} تومان</dd></div></dl></section>
      {booking.service_items.length > 0 && <section className="receipt-payments"><h2>خدمات انتخابی</h2>{booking.service_items.map((item) => <div key={item.slug}><span><b>{item.title}</b><small>{item.status === "requested" ? "در انتظار هماهنگی" : item.status}</small></span><strong>{money(item.total_price)} تومان</strong><code>{item.quantity} عدد</code></div>)}</section>}
      ...
      <footer><p>این رسید از حساب کاربری ویلاوان صادر شده است.</p><a href="/account">بازگشت به رزروهای من</a></footer>
    </article>}
  </section></main>;
}
