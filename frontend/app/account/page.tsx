...
export default function AccountPage() {
  ...
  if (!ready) return <main dir="rtl" className="inner-page account-page"><InnerHeader /><section className="account-access-state section-shell"><span className="status-pulse" /><h1>در حال آماده‌کردن حساب شما…</h1></section></main>;
  if (!authenticated) return <main dir="rtl" className="inner-page account-page"><InnerHeader /><section className="account-access-state section-shell"><span className="brand-mark"><i>V</i><b>1</b></span><h1>برای دیدن سفرها وارد شوید</h1><p>رزروها، پرداخت‌ها و درخواست‌های لغو فقط در حساب امن شما نمایش داده می‌شوند.</p><a href="/login?next=/account">ورود با شماره موبایل</a></section></main>;
  if (loadError) return <main dir="rtl" className="inner-page account-page"><InnerHeader /><section className="account-access-state section-shell"><span className="brand-mark"><i>!</i></span><h1>اطلاعات حساب دریافت نشد</h1><p>{loadError}</p><button type="button" onClick={() => window.location.reload()}>تلاش دوباره</button></section></main>;
  ...
  return <main dir="rtl" className="inner-page account-page">
    <InnerHeader />
